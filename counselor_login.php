...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$terms_agreed) {
        ...
        $query = $conn->prepare("SELECT * FROM counselors WHERE email = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            if (password_verify($password, $counselor['password'])) {
                ...
                $_SESSION['counselor_name'] = $counselor['counselor_name'];
                $_SESSION['email'] = $counselor['email'];
...
<body>
    <div class="flex flex-col justify-center items-center p-6 glass">
        <h2 class="text-2xl font-bold text-gray-100 mb-4">Counselor Login</h2>

        <form method="POST" action="counselor_login.php" class="w-full">
